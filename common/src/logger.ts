...
export class ModuleLogger implements ILogger {
    ...
    constructor(moduleName?: string, parent?: ILogger) {
        ...
        this._moduleName = moduleName || null;
    }

    error(message: any, ...meta: any): void {
        this._parent.error(this.prependModuleName(message), ...meta);
    }
...
export class ConsoleLogger implements ILogger {
    ...
    constructor() {
        ...
        if (typeof global !== "undefined") {
            const logLevel: LogLevel = global.process.env["AIREYE_LOGLEVEL"] || LogLevel.info;
...
export function createLoggerOnConsole(moduleName?: string) {
    if(!moduleName) {
        return console;
    }

    return {
        silly: console.log.bind(console, moduleName),
        debug: console.log.bind(console, moduleName),
        info: console.info.bind(console, moduleName),
        error: console.error.bind(console, moduleName),
        warn: console.warn.bind(console, moduleName),
    }
}

export function createModuleLogger(moduleName?: string, logger?: ILogger) {
    return new ModuleLogger(moduleName, logger);
}

export function createLogger(moduleName?: string, logger?: ILogger): ILogger {
    if(typeof window !== "undefined") {
        return createLoggerOnConsole(moduleName);
    }
