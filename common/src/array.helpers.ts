import { reduceWhitespaces } from "./string.helpers";
...
export function flattenTree<T, K extends keyof T>(items: T[], key: K, res: T[]): T[] {
    for (const item of items) {
        res.push(item);

        const children: T[] = <any>item[key];
        if (children && Array.isArray(children)) {
            flattenTree(children, key, res);
        }
    }
